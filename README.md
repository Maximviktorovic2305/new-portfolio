...
Переменные `VITE_EMAILJS_SERVICE_ID`, `VITE_EMAILJS_TEMPLATE_ID` и `VITE_EMAILJS_PUBLIC_KEY` нужны только для отправки формы. Все переменные с префиксом `VITE_` попадают в клиентский bundle, поэтому в них нельзя хранить приватные ключи.
...
`npm start` раздаёт каталог `dist` на `0.0.0.0:3015` с политиками безопасности из `serve.json`. Workflow деплоя обновляет `/home/max/portfolio` и перезапускает только PM2-процесс `portfolio`.
