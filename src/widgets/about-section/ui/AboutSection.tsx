    ...
    <section id="about" className="relative py-24 overflow-hidden">
      ...
      <div ref={ref} className="max-w-7xl mx-auto px-6">
        ...
        >
          ...
          >
            Кто {isClassic ? "" : <span className="text-brand-teal">я</span>}
            {isClassic && "я"} такой?
          </h2>
          {!isClassic && (
            <motion.div
        ...
        <div className="grid lg:grid-cols-[auto_1fr] gap-10 items-start">
          ...
          >
            ...
            >
              <motion.div
                whileHover={{ scale: isClassic ? 1.02 : 1.04, rotate: 0 }}
            ...
            {isCrayon && (
              <motion.div
                ...
                className="absolute -top-3 -right-3 text-[2rem] select-none"
              >
                📌
              </motion.div>
            )}
          ...
          <div>
            ...
            >
              ...
              — fullstack-разработчик с 5+ годами коммерческого опыта. Работаю на полном стеке: от
              интерактивных интерфейсов на React/Vue до высоконагруженного бэкенда на Node.js и Golang
              {!isClassic && " 💪"}
            </motion.p>
            ...
            <motion.p
              ...
              style={{ fontFamily: "var(--t-font-body)" }}
            >
              Реализовал 40+ проектов — от финтех-платформ до маркетплейсов и админ-панелей.
              {isClassic
            ...
            >
              {hobbies.map((h) => (
                <motion.span
                  key={h.label}
                  whileHover={{
                    scale: isClassic ? 1.04 : isCrayon ? 1.05 : 1.12,
                    y: isClassic ? -2 : -4,
                    rotate: 0,
                  }}
                  whileTap={{ scale: 0.92 }}
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            {highlights.map((item, i) => (
              <motion.div
                ...
                animate={
                  isInView
                    ? { opacity: 1, y: 0, scale: 1, rotate: isCrayon ? [-1.15, 0.75, -0.55, 1][i % 4] : 0 }
                    : {}
                }
                ...
                whileHover={{
                  rotate: 0,
                  y: isCrayon ? -4 : 0,
                  scale: isClassic ? 1.02 : isCrayon ? 1.035 : 1.05,
                }}
                whileTap={{ scale: 0.97 }}
              ...
              >
                <div className="relative z-10 flex h-full flex-col items-center text-center gap-3">
                  <motion.div
                    animate={
                      hoveredHighlight === i ? { y: [0, -3, 0], scale: [1, 1.1, 1] } : { y: 0, scale: 1 }
                    }
                    transition={
                      hoveredHighlight === i
                  ...
                  <div
                    className="text-[1.05rem] text-text-primary"
                    style={{ fontFamily: "var(--t-font-heading)", fontWeight: 700 }}
                  >
                    {item.title}
                  </div>
                  <div
                    className="text-[0.8rem] text-text-secondary leading-relaxed"
                    style={{ fontFamily: "var(--t-font-body)" }}
                  >
                    {item.desc}
                  </div>
                ...
                <AnimatePresence>
                  {hoveredHighlight === i && (
                    <motion.div
                      ...
                      style={{
                        background: `linear-gradient(90deg, transparent, ${item.color}, ${colors.orange}, ${item.color}, transparent)`,
                      }}
                    />
                  )}
                </AnimatePresence>
            ...
            {stats.map((stat, i) => (
              <motion.div
                ...
                initial={{
                  ...
                  y: 30,
                  scale: 0.9,
                  rotate: isCrayon ? [1.6, -1.4, 1, -1.8][i % 4] : 0,
                }}
                animate={
                  isInView
                    ? { opacity: 1, y: 0, scale: 1, rotate: isCrayon ? [0.8, -0.7, 0.5, -0.9][i % 4] : 0 }
                    : {}
                }
              ...
              >
                ...
                <div
                  className="relative z-10 text-[0.7rem] text-text-secondary uppercase tracking-wider"
                  style={{ fontFamily: "var(--t-font-body)" }}
                >
                  {stat.label}
                </div>
                ...
                <AnimatePresence>
                  {hoveredStat === i && (
                    <motion.div
                      ...
                      className="absolute bottom-0 left-0 right-0 h-[0.1875rem] origin-center rounded-full"
                      style={{
                        background: `linear-gradient(90deg, transparent, ${stat.color}, ${colors.orange}, ${stat.color}, transparent)`,
                      }}
                    />
                  )}
                </AnimatePresence>
